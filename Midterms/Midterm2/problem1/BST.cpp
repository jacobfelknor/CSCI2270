...
int BST::sumRange(int min, int max){
  ...
  sumRangeHelper(root, sum, min, max);

  return sum;

}

void BST::sumRangeHelper(Node *currNode, int &sum, int min, int max){
  if(currNode)
     {
        sumRangeHelper(currNode->left, sum, min, max); 
        if(currNode->key <= max && currNode->key >= min)
          sum = sum + currNode->key;
        sumRangeHelper(currNode->right, sum, min, max);
     }
}